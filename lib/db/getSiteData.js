...
const EmptyData = pageId => {
  const empty = {
    ...
    allPages: [
      {
        id: 1,
        title: `Unable to obtain Notion data, please check Notion_ID： \n current ${pageId}`,
        summary:
          'Visit the documentation for help → https://docs.tangly1024.com/article/vercel-deploy-notion-next',
        status: 'Published',
        type: 'Post',
        slug: 'oops',
  ...
  if (POST_SCHEDULE_PUBLISH) {
    ...
    db.allPages?.forEach(p => {
      ...
      if (!publish) {
        ...
        console.log(
          '[Scheduled Release] Hide-->Article:',
          p.title,
          'Current timestamp:',
          currentTimestamp,
          'Target Timestamp:',
...
function getSiteInfo({ collection, block, NOTION_CONFIG }) {
  ...
  const defaultDescription =
    NOTION_CONFIG?.DESCRIPTION || 'This is a site generated by Hemal Mondal'
