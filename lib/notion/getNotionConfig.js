...
export async function getConfigMapFromConfigPage(allPages) {
  ...
  if (pageIds?.length === 0) {
    console.error(
      '[Notion configuration] The article list obtained is empty, please check the notion template',
      collectionQuery,
      collection,
      collectionView,
      viewIds,
  ...
  try {
    ...
    console.warn('An error occurred while parsing the INLINE_CONFIG configuration. Please check the JSON format.', err)
  }
  return combine
}
...
export function parseConfig(configString) {
  ...
  try {
    ...
    console.warn(
      'Error parsing eval (INLINE_CONFIG) configuration, please check the JSON format',
      evalError
    )
    return {}
  }
